...
public class UserServiceImpl implements UserService {
    ...
    public void addFriend(long userId, long friendId) {
        final User user = repository.getById(userId)
                .orElseThrow(() -> {
                    log.warn("ADD-FRIEND {}<->{}. Пользователь с айди {} не найден", userId, friendId, userId);
                    return new NotFoundException("Пользователь с id=" + userId + " не существует");
                });
        final User friend = repository.getById(friendId)
                .orElseThrow(() -> {
                    log.warn("ADD-FRIEND {}<->{}. Пользователь с айди {} не найден", userId, friendId, friendId);
    ...
    public Collection<User> getFriends(long userId) {
        final User user = repository.getById(userId)
                .orElseThrow(() -> {
                    log.warn("GET-FRIENDS {}. Пользователь с айди {} не найден", userId, userId);
                    return new NotFoundException("Пользователь с id=" + userId + " не существует");
                });
    ...
    public Collection<User> getCommonFriends(long userId, long otherId) {
        final User user = repository.getById(userId)
                .orElseThrow(() -> {
                    log.warn("GET-COMMON-FRIENDS {}<->{}. Пользователь с айди {} не найден", userId, otherId, userId);
                    return new NotFoundException("Пользователь с id=" + userId + " не существует");
                });
        final User other = repository.getById(otherId)
                .orElseThrow(() -> {
                    log.warn("GET-COMMON-FRIENDS {}<->{}. Пользователь с айди {} не найден", userId, otherId, otherId);
